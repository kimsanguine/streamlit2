...
def build_index(chunks: tuple[str, ...]):
    ...
    # [왜] sklearn은 import에 1~2초 — 검색을 실제 수행할 때만 불러 화면 첫 표시를 빠르게 한다.
...
def _extract_pdf_text(file_bytes: bytes) -> str:
    ...
    # [왜] 암호가 걸렸거나 손상된 PDF는 파싱 단계에서 예외를 던진다 — 앱을 중단시키는 대신
    #      빈 문자열로 돌려, 호출부의 "추출 실패" 안내 흐름에 합류시킨다.
    try:
...
def main():
    ...
    if uploaded.name.lower().endswith(".pdf"):
        ...
        if not raw_text.strip():
            st.warning("⚠️ 텍스트를 추출하지 못했습니다 — 스캔본(이미지) PDF는 OCR이 별도로 필요하고, 암호가 걸렸거나 손상된 PDF는 열 수 없습니다.")
            return
    else:
