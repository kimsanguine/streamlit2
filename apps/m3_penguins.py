...
def train_model(df, model_type, n_estimators, max_depth):
    ...
    #      교차검증의 평균±표준편차가 "분할에 따라 점수가 얼마나 출렁이는지"(변동성)를 함께 보여준다.
...
def main():
    ...
    with tab_perf:
        ...
        with st.expander("🔬 심화: 교차검증 변동성 + 특징 중요도 더 알아보기"):
            st.markdown("**교차검증(cross-validation) 정확도 — 분할에 따라 얼마나 출렁이나**")
            st.metric("5-fold 교차검증 평균", f"{out['cv_mean']:.1%} ± {out['cv_std']:.1%}")
            st.caption(
                "단일 split의 test_acc는 seed에 따라 흔들리는 점추정 — 5-fold 층화(StratifiedKFold)로 "
                "5번 다시 나눠 평균±표준편차를 보면 분할에 따라 점수가 얼마나 출렁이는지(변동성)를 참고할 수 있다."
            )
