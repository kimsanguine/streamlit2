...
def _tool_analyze_sentiment(text: str) -> str:
    # [왜] m4_sentiment는 transformers(import 5초+)를 끌고 온다 — 파일 최상단이 아니라 도구가 실제
    #      호출될 때 import해야 챗봇 화면이 즉시 뜬다(감성 도구를 안 쓰는 대화는 아예 로드 비용 0).
    from apps.m4_sentiment import analyze as _analyze_sentiment  # ST3 감성분석 모델 재사용
